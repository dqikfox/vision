...
ESC = "\033["
R = f"{ESC}0m"
B = f"{ESC}38;2;59;130;246m"
C = f"{ESC}38;2;6;182;212m"
G = f"{ESC}38;2;34;197;94m"
Y = f"{ESC}38;2;251;191;36m"
D = f"{ESC}38;2;71;85;105m"
RD = f"{ESC}38;2;255;71;87m"
BLD = f"{ESC}1m"
W = 64
...
def _speak_local(text: str):
    _get_pyttsx().say(text)
    _get_pyttsx().runAndWait()
...
def _stop_and_process():
    ...
    if not frames:
        status("LISTEN", f"{D}No audio{R}", D)
        return
    ...
    if not text:
        status("WHISPER", f"{D}[silence or unclear]{R}", D)
        return
...
def _on_rctrl(ev):
    ...
    if ev.event_type == keyboard.KEY_DOWN and not _recording:
        ...
        threading.Thread(target=_draw_vu, args=(_audio_frames, _vu_stop_event), daemon=True).start()
