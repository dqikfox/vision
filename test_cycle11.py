...
class TestBrowserEvalToolErr:
    ...
    async def test_browser_eval_timeout_logged(self):
        ...
        with (
            mock.patch.object(app, "get_browser_page", return_value=page_mock),
...
class TestConfigCorruptionDeleteFallback:
    def test_corrupt_config_rename_fails_then_deletes(self, tmp_path):
        ...
        with (
            mock.patch.object(app, "COMMAND_CENTER_CONFIG_FILE", bad_file),
            mock.patch.object(bad_file, "rename", fake_rename),
            mock.patch.object(app, "write_log", side_effect=lambda *a: log_calls.append(a)),
        ):
