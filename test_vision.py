...
async def test_ws():
    ...
    try:
        async with websockets.connect(WS, open_timeout=5) as ws:
            ...
            ok("WS connect + init received", f"provider={msg.get('provider')} model={msg.get('model')}")
    except Exception as e:
    ...
    while time.time() < deadline:
        try:
            async with websockets.connect(WS, open_timeout=5) as ws:
                ...
                await ws.send(
                    json.dumps(
                        {"type": "input", "text": "Reply with exactly this text and nothing else: VISION_TEST_OK"}
                    )
                )
                while time.time() < deadline:
                    try:
...
async def test_tools():
    ...
    if r and "ERROR" not in r:
        wins = [ln for ln in r.splitlines() if ln.strip()]
    ...
    try:
        ...
        async with websockets.connect(WS, open_timeout=5) as ws:
            ...
            try:
                ...
                await ws.send(
                    json.dumps({"type": "input", "text": "Run the echo command to output the word TOOLCALL_OK"})
                )
                deadline = time.time() + 60
                got_action = False
                got_reply = False
                ...
                if not got_action:
                    fail(
                        "Ollama operator tool call",
                        "No tool action seen — model may not support native FC for this task",
                    )
            finally:
